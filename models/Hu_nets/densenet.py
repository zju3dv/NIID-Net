...
MODEL_DIR = 'models/pretrained/encoders'
...
def densenet161(pretrained=False, **kwargs):
    ...
    model = DenseNet(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24),
                     **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['densenet161'], MODEL_DIR))
    return model
...
def densenet121(pretrained=False, **kwargs):
    ...
    model = DenseNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['densenet121'], MODEL_DIR))
    return model
