...
class CGIntrinsicsImageFolder(data.Dataset):
    ...
    def load_CGIntrinsics(self, path):
        ...
        return srgb_img, rgb_img, gt_R, gt_S, mask, random_filp
    ...
    def __getitem__(self, index):
        ...
        srgb_img, rgb_img, gt_R, gt_S, mask, random_filp = self.load_CGIntrinsics(img_path)
        ...
        # intensity
        # targets_1['R_ints'] = torch.mean(targets_1['gt_R'] / targets_1['chromaticity'], dim=0, keepdim=True)
        if np.sum(mask) < 10:
            max_S = 1.0
        else:
            max_S = np.percentile(gt_S[mask > 0.5], 90)
        gt_S_ints = gt_S/max_S
        gt_S_ints = np.mean(gt_S_ints, 2)
        gt_S_ints = np.expand_dims(gt_S_ints, axis=2)
        gt_R_ints = np.mean(gt_R, 2)
        gt_R_ints = np.expand_dims(gt_R_ints, axis=2)
        targets_1['gt_R_ints'] = torch.from_numpy(np.transpose(gt_R_ints, (2 , 0 ,1))).contiguous().float()
        targets_1['gt_S_ints'] = torch.from_numpy(np.transpose(gt_S_ints, (2 , 0 ,1))).contiguous().float()
...
class Render_ImageFolder(data.Dataset):
    ...
    def load_Render(self, path):
        ...
        # rgb_img = srgb_img**2.2
        rgb_img = self.DA(rgb_img, 1, random_pos, random_filp, h, w)
        ...
        # gt_R = np.mean(gt_R,2)
        # gt_R = np.expand_dims(gt_R, axis = 2)
        return srgb_img, rgb_img, gt_R, gt_S, mask
    ...
    def __getitem__(self, index):
        ...
        # split_img_path = img_path.split('/')
        full_path = self.root + img_path
        srgb_img, rgb_img, gt_R, gt_S, mask = self.load_Render(img_path)
        ...
        targets_1['gt_R_ints'] = torch.from_numpy(np.transpose(gt_R_ints, (2 , 0 ,1))).contiguous().float()
        targets_1['gt_S_ints'] = torch.from_numpy(np.transpose(gt_S_ints, (2 , 0 ,1))).contiguous().float()
        # targets_1['R_ints'] = torch.mean(targets_1['gt_R'], dim=0, keepdim=True)
        # targets_1['S_ints'] = torch.mean(targets_1['gt_S'], dim=0, keepdim=True)
